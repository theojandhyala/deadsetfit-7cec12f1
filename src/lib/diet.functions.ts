...
{"name":"...","calories":0,"protein":0,"carbs":0,"fats":0,"confidence":"low|medium|high","notes":"..."}`;
    return await chatVisionJSON<{ name: string; calories: number; protein: number; carbs: number; fats: number; confidence: string; notes: string }>({
      system: sys,
...
Where adherence and hydrationScore are 0-100.`;
    ...
    return await chatJSON<{ grade: string; adherence: number; avgCalories: number; avgProtein: number; hydrationScore: number; wins: string[]; misses: string[]; action: string }>({
      system: sys,
      user,
    });
  });
