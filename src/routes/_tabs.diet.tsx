...
import { Loader2, RefreshCw, Plus, Camera, ScanBarcode, Droplets, FileBarChart, Bell, BellOff, Trash2 } from "lucide-react";
...
function DietPage() {
  ...
  const gen = generateMeals;
  const swap = swapMeal;
  const analyze = analyzeFoodPhoto;
  const barcode = lookupBarcode;
  const report = weeklyNutritionReport;
  ...
  function addFood() {
    ...
    const item: FoodLogItem = { date: today, name, calories: c, protein: 0, carbs: 0, fats: 0, source: "manual" };
  ...
  async function doBarcode() {
    ...
    setBarcodeLoad(true); setError(null);
    try {
  ...
  async function generateReport() {
    if (!profile) return;
    setReportLoad(true); setError(null);
    try {
      ...
      const r = await report({ data: { goal: profile.goal, targetCalories: calories, targetProtein: macros.protein, days } });
      setWeekly(r);
  ...
  return (
    <div style={{ paddingTop: "env(safe-area-inset-top)" }}>
      ...
      <section id="food-log-section" className="px-5 mb-6">
        ...
        <div className="bg-grit-card border border-grit p-4">
          ...
          <div className="flex gap-2 mb-3">
            ...
            <input inputMode="numeric" value={cals} onChange={(e) => setCals(e.target.value)} placeholder="kcal" className="input-grit w-20" />
              ...
              {f.photoThumb ? (
                ...
                <div className="w-9 h-9 bg-[#1a1a1a] border border-grit flex items-center justify-center flex-shrink-0">
                  {f.source === "barcode" ? <ScanBarcode size={12} className="text-[#8a8a8a]" /> :
                   f.source === "photo" ? <Camera size={12} className="text-[#8a8a8a]" /> :
      ...
      <section className="px-5 mb-6">
        <div className="flex items-center justify-between mb-2">
          <p className="label-cap flex items-center gap-2"><FileBarChart size={12} /> Weekly Report</p>
          <button onClick={generateReport} disabled={reportLoad} className="label-cap text-accent-red">
        ...
        {weekly && (
          <div className="bg-grit-card border border-grit p-5">
            <div className="flex items-end gap-4 mb-4">
              <div className="display font-extrabold text-grit text-6xl leading-none">{weekly.grade}</div>
              <div className="flex-1">
                <p className="label-cap text-accent-red">Adherence</p>
                <div className="h-2 bg-[#262626] mt-1"><div className="h-full bg-accent-red" style={{ width: `${weekly.adherence}%` }} /></div>
            ...
            <div className="mb-3">
              ...
              {weekly.wins.map((w, i) => <p key={i} className="text-xs text-grit">+ {w}</p>)}
            </div>
            <div className="mb-3">
              <p className="label-cap text-[#8a8a8a] mb-1">Misses</p>
              {weekly.misses.map((w, i) => <p key={i} className="text-xs text-[#8a8a8a]">— {w}</p>)}
            </div>
