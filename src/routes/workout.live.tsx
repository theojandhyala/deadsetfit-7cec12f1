...
function LiveWorkoutPage() {
        ...
        {active ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-sm text-[#8a8a8a] mb-6">
              Pick or build a program to start training.
            </p>
            <Link to="/programs" className="btn-grit">
              Browse Programs
            </Link>
          </div>
        )}
  ...
  const progress = Math.min(
    ...
    Math.round(
      (totals.sets /
        Math.max(
          1,
          session.exercises.reduce((a, e) => a + e.targetSets, 0),
        )) *
        100,
    ),
  );
  ...
  function logSet(weight: number, reps: number, opts?: { rpe?: number; isAmrap?: boolean }) {
    ...
    updateSession((sess) => {
      const exercises = sess.exercises.map((e, i) =>
    ...
    >
      ...
      <div className="flex-1 px-5 py-4 overflow-auto">
        ...
        <div className="mt-4 flex flex-col gap-2">
          {current.sets.map((s, i) => (
            <div
              key={i}
              className="grid grid-cols-[40px_1fr_1fr_auto_auto] items-center gap-2 bg-grit-card border border-grit px-3 py-2.5"
            >
              <div className="label-cap text-grit-dim">SET {i + 1}</div>
              <div>
                <span className="display text-xl font-extrabold text-grit">{s.weight}</span>
                <span className="text-xs text-[#8a8a8a] ml-1">kg</span>
              </div>
              <div>
                <span className="display text-xl font-extrabold text-grit">{s.reps}</span>
                <span className="text-xs text-[#8a8a8a] ml-1">reps</span>
              </div>
              <div className="flex items-center gap-1 text-[10px] text-grit-dim">
                {s.isAmrap && (
                  <span className="px-1 py-0.5 border border-grit-dim text-grit-dim">AMRAP</span>
                )}
        ...
        {current.sets.length > 0 && (
          <button
            ...
            className="mt-3 label-cap text-grit-dim text-xs flex items-center mx-auto"
          >
      ...
      {restLeft > 0 && (
        <div className="sticky bottom-0 mx-4 mb-4 bg-grit-card border-2 border-accent-red p-3 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => setRestLeft((s) => s + 15)} className="btn-ghost">
              <Plus size={14} className="mr-1" />
              15s
            </button>
            <button onClick={() => setRestLeft(0)} className="btn-grit">
              Skip
            </button>
          </div>
        </div>
      ...
      <div className="border-t border-grit p-4 grid grid-cols-2 gap-3">
        ...
        <button
          ...
          disabled={scoring || totals.sets === 0}
          className="btn-grit"
        >
          {scoring ? (
...
function SetEntry({
  ...
  useEffect(() => {
    if (prev) {
      setW(String(prev.weight));
      setR(String(prev.reps));
    }
  }, [prev]);
  const wn = Number(w) || 0;
  const rn = Number(r) || 0;
      ...
      {lastSessionSet && (
        <div className="flex items-center justify-between mb-2 gap-2">
          <p className="text-[11px] text-grit-dim uppercase tracking-wider truncate">
            Last time:{" "}
            <span className="text-grit font-bold">
              {lastSessionSet.weight}kg × {lastSessionSet.reps}
            </span>
          </p>
          {progression > 0 && (
            <button
              onClick={() => {
                setW(String(progression));
                setR(String(suggestedReps || prev?.reps || 5));
              }}
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <button
              onClick={() => onOpenPlateCalc(wn)}
              className="label-cap text-accent-red flex items-center text-[10px]"
            >
              <Calculator size={11} className="mr-1" />
              Plates
            </button>
          </div>
          <div className="flex items-center">
            <button
              onClick={() => setW(String(Math.max(0, (Number(w) || 0) - 2.5)))}
              className="w-10 h-12 border border-grit text-grit"
            >
              −
            </button>
            <input
              ...
              className="input-grit text-center flex-1 mx-0 border-l-0 border-r-0"
            />
            <button
              onClick={() => setW(String((Number(w) || 0) + 2.5))}
              className="w-10 h-12 border border-grit text-grit"
            >
              +
            </button>
        ...
        <div>
          ...
          <div className="flex items-center">
            <button
              onClick={() => setR(String(Math.max(0, (Number(r) || 0) - 1)))}
              className="w-10 h-12 border border-grit text-grit"
            >
              −
            </button>
            ...
            />
            <button
              onClick={() => setR(String((Number(r) || 0) + 1))}
              className="w-10 h-12 border border-grit text-grit"
            >
              +
            </button>
      ...
      <div className="flex items-center justify-between mb-3 text-[11px]">
        ...
        {e1rm > 0 && (
          <span className="text-grit-dim">
            e1RM <span className="text-grit font-bold">{e1rm}kg</span>
          </span>
        )}
      </div>
        ...
        {session.pumpScore != null && (
          <div className="mt-8 border-2 border-accent-red p-5 bg-grit-card">
            ...
            <div className="flex items-end gap-3 mt-1">
              <p className="display text-[88px] leading-none font-extrabold text-grit">
                {session.pumpScore}
              </p>
              <p className="text-xs text-grit-dim pb-3">/ 100</p>
            </div>
            {session.pumpNote && (
              <p className="display text-sm uppercase font-bold text-grit mt-2">
                "{session.pumpNote}"
              </p>
            )}
          </div>
