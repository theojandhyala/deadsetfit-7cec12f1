...
function ProExperienceStage() {
  const unit = useUnit();
  // 2.5 kg is a pair of the smallest plates; 5 lb is the same idea in a pound
  // gym. Showing a kilogram step to a pound athlete on the page asking them to
  // pay is exactly the wrong place to look like someone else's app.
  const step = `+${increment(unit)} ${unit}`;
  return (
    <div className="deadset-pro-stage" aria-label="DEADSET Pro strength map and live programme">
      ...
      <div className="deadset-pro-plan-card">
        ...
        <div className="mt-3 rounded-lg border border-emerald-400/20 bg-emerald-400/[0.07] px-2 py-1.5 text-left">
          ...
          <p className="mt-0.5 text-[9px] font-black text-white">Bench press · {step}</p>
        </div>
      </div>
    </div>
  );
