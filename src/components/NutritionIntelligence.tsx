...
export function NutritionIntelligence({
  ...
  const r = useMemo(() => {
    ...
    const calHit = logged.filter((x) => x.cal >= calorieTarget * 0.9 && x.cal <= calorieTarget * 1.1).length;
    const pHit = logged.filter((x) => x.p >= proteinTarget * 0.9).length;
    const adherence = Math.round(((calHit / logged.length) * 0.5 + (pHit / logged.length) * 0.5) * 100);
  ...
  let aligned = true;
  if (goal === "CUT") {
    if (r.balance > 0) { verdict = "You're in a surplus — tighten calories to actually cut."; aligned = false; }
    else verdict = `On track — about ${Math.abs(r.weeklyKg)}kg/week down at this pace.`;
  } else if (goal === "BULK") {
    if (r.balance < 0) { verdict = "You're in a deficit — add calories to grow."; aligned = false; }
    else verdict = `Fueling growth — about ${Math.abs(r.weeklyKg)}kg/week up at this pace.`;
  } else {
    if (Math.abs(r.balance) > 250) { verdict = `Drifting ${r.balance > 0 ? "over" : "under"} maintenance — nudge back to hold weight.`; aligned = false; }
    else verdict = "Holding maintenance well.";
  }

  const proteinShort = proteinTarget - r.avgP;
  const tip = proteinShort > 15
  ...
  const balColor = goal === "CUT" ? (r.balance <= 0 ? "#22c55e" : "#e63222")
    : goal === "BULK" ? (r.balance >= 0 ? "#22c55e" : "#e63222")
    : (Math.abs(r.balance) <= 250 ? "#22c55e" : "#f59e0b");
    ...
    <div>
      <div className="flex items-center gap-4 mb-3">
        <div>
          ...
          <p className="display text-3xl font-extrabold tabular-nums" style={{ color: r.adherence >= 75 ? "#22c55e" : r.adherence >= 50 ? "#f59e0b" : "#e63222" }}>
            {r.adherence}
        ...
        <div className="flex-1 border-l border-grit pl-4">
          ...
          <p className="display text-xl font-extrabold flex items-center gap-1" style={{ color: balColor }}>
            <BalIcon size={16} />
            {r.balance > 0 ? "+" : ""}{r.balance}
            <span className="text-[10px] text-grit-dim">kcal/day</span>
          </p>
          <p className="text-[10px] text-grit-dim mt-0.5">
            ≈ {r.weeklyKg > 0 ? "+" : ""}{r.weeklyKg}kg/week
          </p>
        </div>
      </div>
      <div className="flex items-start gap-2 rounded-xl border p-3" style={{ borderColor: aligned ? "#1c3a24" : "#3a1a10", background: aligned ? "#0e1a12" : "#180d0a" }}>
        <Target size={14} className="mt-0.5 shrink-0" style={{ color: aligned ? "#22c55e" : "#f59e0b" }} />
        <div>
