      ...
      <div className="bg-grit-card border border-grit rounded-2xl p-4 relative overflow-hidden">
        ...
        <div className="flex items-baseline gap-2 mt-1.5">
          <p className="display text-3xl font-extrabold text-grit leading-none">
            {Math.round(toDisplay(stats.totalVolumeKg, unit)).toLocaleString()}
          </p>
          <p className="label-cap text-[9px] text-grit-dim">{unit} lifted, lifetime</p>
        </div>
        {stats.equivalent && (
        ...
        {(stats.heaviestSet || stats.topExercise) && (
          <p className="text-[11px] text-grit-dim leading-relaxed mt-3">
            {stats.heaviestSet && (
              <>
                Heaviest set:{" "}
                <span className="text-grit">{formatWeight(stats.heaviestSet.weight, unit)}</span> ×{" "}
